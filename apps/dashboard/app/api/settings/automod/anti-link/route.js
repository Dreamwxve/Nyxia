...
export async function POST(request) {
 try {
   ...
   data.actions = [
    {
     ...
     metadata: {
      custom_message: "Message blocked due to containing an link from the default filter.",
     },
    },
   ];
  }
  ...
  const validatedActions = await validateAutoModRuleActions(data.actions, allChannels, "Message blocked due to containing an link from the default filter.");
